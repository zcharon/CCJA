...
class MyBertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query = nn.Linear(config.hidden_size, self.all_head_size)  # Q
        self.key = nn.Linear(config.hidden_size, self.all_head_size)  # K
        self.value = nn.Linear(config.hidden_size, self.all_head_size)  # V
        self.config = config
    ...
    def p_init(self, input_length, batch_size = 1, init_mag = 1.0, cuda_device = 0, perturb_mask = None):
        ...
        self.perturb = torch.zeros((batch_size, input_length, self.config.hidden_size), device = cuda_device).uniform_(-init_mag, init_mag)/SQRT_NUMEL  
    ...
    def p_accu(self, loss, adv_lr = 1.0, input_length = None, average_input_length = 20, project_to = None, perturb_mask = None):
        grad = torch.autograd.grad(loss, self.perturb)  
        grad = grad[0]
    ...
    def p_accu_adam(self, loss, adv_lr = 1.0, input_length = None, average_input_length = 20, project_to = None, perturb_mask = None):
        beta1 = 0.9  
        beta2 = 0.999 
        epsilon = 1e-8 
        ...
        if not hasattr(self, 'm_t'):
            self.v_t = torch.zeros_like(self.perturb)
            self.s_t = torch.zeros_like(self.perturb)
            self.t = 0

        self.t += 1
        self.v_t = beta1 * self.v_t + (1 - beta1) * grad
        self.s_t = beta2 * self.s_t + (1 - beta2) * (grad**2)
        v_hat = self.v_t / (1 - beta1**self.t)
        s_hat = self.s_t / (1 - beta2**self.t)
        grad = adv_lr * v_hat / (torch.sqrt(s_hat) + epsilon)
        self.perturb = (self.perturb + grad).detach()
        if project_to is not None:
            if self.perturb.norm() > project_to:
                self.perturb = (project_to * self.perturb / self.perturb.norm()).detach()
...
class MyAlbertTransformer(nn.Module):
    ...
    def p_init(self, input_length, batch_size = 1, init_mag = 1.0, cuda_device = 0, perturb_mask = None):
        # 为embedding space添加初始扰动
        SQRT_NUMEL = input_length * self.config.hidden_size
        self.perturb = torch.zeros((batch_size, input_length, self.config.hidden_size), device = torch.device('cuda:' + str(cuda_device))).uniform_(-init_mag, init_mag)/SQRT_NUMEL # 设置(batch_size, sentence_length, hidden_size)尺寸的，归一化后的均匀分布的扰动
